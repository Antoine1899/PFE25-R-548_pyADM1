...
k_dis = 1.5 #d^-1 / 0.5 initialement
k_hyd_ch =  0.5 #d^-1 / 10 initialement
k_hyd_pr =  0.4 #d^-1 / 10 initialement
k_hyd_li =  0.2 #d^-1 / 10 initialement
K_S_IN =  10 ** -4 #M
k_m_su =  30 #d^-1
...
# -------------------------------------------------------------
influent_state = pd.read_csv("digester_influent_test9.csv", sep=';')
...
q_ad = influent_state['Q'][0] #m^3.d^-1 initial flow rate (can be modified during the simulation by the control algorithm)
...
# ==============================================================================
# 5. POST-TRAITEMENT & SAUVEGARDE (CORRIGÉ AVEC CUMUL)
# ==============================================================================
...
# Ajout du débit instantané q_ch4 (m3/j)
...
simulate_results['V_ch4_cumul'] = ((simulate_results['q_ch4'] * dt)/172.96).cumsum()
# -------------------------------------

# Sauvegarde du fichier CSV complet
simulate_results.to_csv("dynamic_out_test9.csv", index = False, sep=';')
...
try:
    ...
    # --- MODIFICATION ICI : Calcul de la somme des AGV ---
